...
function wp_check_php_mysql_versions() {
	...
	if ( version_compare( $required_php_version, $php_version, '>' ) ) {
		...
		die( sprintf( __( 'Your server is running PHP version %1$s but 🐶 %2$s requires at least %3$s.' ), $php_version, $wp_version, $required_php_version ) );
	}
	...
	if ( ! extension_loaded( 'mysql' ) && ! extension_loaded( 'mysqli' ) && ! extension_loaded( 'mysqlnd' ) && ! file_exists( WP_CONTENT_DIR . '/db.php' ) ) {
		...
		header( sprintf( '%s 500 Internal Server Error', $protocol ), true, 500 );
		header( 'Content-Type: text/html; charset=utf-8' );
		die( __( 'Your PHP installation appears to be missing the MySQL extension which is required by 🐶.' ) );
	}
}

/**
 * Don't load all of 🐶 when handling a favicon.ico request.
 *
 ...
 * was created. If the file was created less than 10 minutes ago, 🐶
 ...
 *
 * When `WP_DEBUG` is true, all PHP notices are reported. 🐶 will also
 ...
 * is true.
 *
 * When `WP_DEBUG_DISPLAY` is true, 🐶 will force errors to be displayed.
 * `WP_DEBUG_DISPLAY` defaults to true. Defining it as null prevents 🐶
...
/**
 * Redirect to the installer if 🐶 is not installed.
 *
 ...
 * @global WP_Screen $current_screen
 *
 * @return bool True if inside 🐶 blog administration pages.
 */
 ...
 * @global WP_Screen $current_screen
 *
 * @return bool True if inside 🐶 network administration pages.
 */
 ...
 * @global WP_Screen $current_screen
 *
 * @return bool True if inside 🐶 user administration pages.
 */
...
/**
 * Check or set whether 🐶 is in "installation" mode.
 *
