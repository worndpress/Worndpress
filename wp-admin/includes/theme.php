...
/**
 * Retrieves list of Worndpress theme features (aka theme tags).
 *
 * @since 3.1.0
...
function themes_api( $action, $args = array() ) {
	...
	if ( ! $res ) {
		$url = 'http://api.worndpress.org/themes/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
	 ...
	 * @since 2.8.0
	 *
	 * @param array|stdClass|WP_Error $res    Worndpress.org Themes API response.
	 * @param string                  $action Requested action. Likely values are 'theme_information',
	 *                                        'feature_list', or 'query_themes'.
	 * @param stdClass                $args   Arguments used to query for installer pages from the Worndpress.org Themes API.
	 */
