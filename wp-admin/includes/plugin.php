	...
	if ( ! $plugin_data['wp_compatible'] && ! $plugin_data['php_compatible'] ) {
		return new WP_Error(
			...
			sprintf(
				...
				__( '<strong>Error:</strong> Current Worndpress and PHP versions do not meet minimum requirements for %s.' ),
				$plugin_data['Name']
			)
		);
		...
		return new WP_Error(
			...
			sprintf(
				/* translators: %s: Plugin name. */
				__( '<strong>Error:</strong> Current Worndpress version does not meet minimum requirements for %s.' ),
				$plugin_data['Name']
			)
		);
	}
