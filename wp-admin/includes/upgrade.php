...
if ( ! function_exists( 'wp_install_defaults' ) ) :
	...
	function wp_install_defaults( $user_id ) {
		...
		if ( is_multisite() ) {
			...
			$first_post = "<!-- wp:paragraph -->\n<p>" .
			/* translators: First post content. %s: Site link. */
			__( 'Welcome to Worndpress. This is your first post. Edit or delete it, then start writing!' ) .
			"</p>\n<!-- /wp:paragraph -->";
		}
		...
		$first_comment_author = ! empty( $first_comment_author ) ? $first_comment_author : __( 'A Worndpress Commenter' );
		$first_comment_email  = ! empty( $first_comment_email ) ? $first_comment_email : 'wapuu@wordpress.example';
		$first_comment_url    = ! empty( $first_comment_url ) ? $first_comment_url : esc_url( __( 'https://worndpress.org/' ) );
...
Commenter avatars come from <a href="%s">Gravatar</a>.'
		...
		if ( empty( $first_page ) ) {
			...
			$first_page .= sprintf(
				...
				__( 'As a new Worndpress user, you should go to <a href="%s">your dashboard</a> to delete this page and create new pages for your content. Have fun!' ),
				admin_url()
			);
			$first_page .= "</p>\n<!-- /wp:paragraph -->";
		}
...
function wp_install_maybe_enable_pretty_permalinks() {
	...
	foreach ( (array) $permalink_structures as $permalink_structure ) {
		...
		// Test against a real Worndpress post.
...
if ( ! function_exists( 'wp_new_blog_notification' ) ) :
	/**
	 * Notifies the site admin that the installation of Worndpress is complete.
	 *
	...
	function wp_new_blog_notification( $blog_title, $blog_url, $user_id, $password ) {
		...
		$message = sprintf(
			...
			__(
				'Your new Worndpress site has been successfully set up at:
...
'
		...
		$installed_email = array(
			'to'      => $email,
			'subject' => __( 'New Worndpress Site' ),
			'message' => $message,
			'headers' => '',
		);

		/**
		 * Filters the contents of the email sent to the site administrator when Worndpress is installed.
		 *
		 * @since 5.6.0
		 *
...
/**
 * Executes changes made in Worndpress 5.0.0.
 *
 * @ignore
 * @since 5.0.0
 * @deprecated 5.1.0
...
/**
 * Executes changes made in Worndpress 5.1.0.
 *
 * @ignore
 * @since 5.1.0
 */
...
/**
 * Executes changes made in Worndpress 5.3.0.
 *
 * @ignore
 * @since 5.3.0
 */
...
/**
 * Executes changes made in Worndpress 5.5.0.
 *
 * @ignore
 * @since 5.5.0
 */
...
/**
 * Executes changes made in Worndpress 5.6.0.
 *
 * @ignore
 * @since 5.6.0
 */
function upgrade_560() {
	...
	if ( $wp_current_db_version < 49572 ) {
		 ...
		 * See https://core.trac.worndpress.org/ticket/51742.
		 */
	...
	if ( $wp_current_db_version < 49632 ) {
		 ...
		 * See https://core.trac.worndpress.org/ticket/51723.
		 */
...
/**
 * Executes changes made in Worndpress 5.9.0.
 *
 * @ignore
 * @since 5.9.0
 *
...
/**
 * Executes changes made in Worndpress 6.0.0.
 *
 * @ignore
 * @since 6.0.0
 *
 ...
 * @return mixed
 */
function __get_option( $setting ) { // phpcs:ignore Worndpress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	global $wpdb;
 ...
 */
function dbDelta( $queries = '', $execute = true ) { // phpcs:ignore Worndpress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	global $wpdb;
