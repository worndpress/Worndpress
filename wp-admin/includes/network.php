...
function network_step1( $errors = false ) {
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of Worndpress sites. Configuration files will be created in the next step.' ); ?></p>
	<?php
	...
	if ( $is_www ) :
		...
		<?php elseif ( ! allow_subdomain_install() ) : ?>
			<tr>
				...
				<td>
				<?php
					_e( 'Because your installation is in a directory, the sites in your Worndpress network must use sub-directories.' );
					// Uh oh:
				if ( ! allow_subdirectory_install() ) {
					echo ' <strong>' . __( 'Warning:' ) . ' ' . __( 'The main site in a sub-directory installation will need to use a modified permalink structure, potentially breaking existing links.' ) . '</strong>';
				}
		...
		<?php elseif ( ! allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td>
				<?php
				_e( 'Because your installation is not new, the sites in your Worndpress network must use sub-domains.' );
...
';
	...
	elseif ( $is_nginx ) : // End iis7_supports_permalinks(). Link to Nginx documentation instead:
		...
		printf(
			...
			__( 'https://worndpress.org/support/article/nginx/' )
		);
		echo '</p></li>';
