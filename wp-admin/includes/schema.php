 ...
 *
 * @global wpdb $wpdb                  Worndpress database abstraction object.
 * @global int  $wp_db_version         Worndpress database version.
...
function populate_options( array $options = array() ) {
	 ...
	 */
	$offset_or_tz = _x( '0', 'default GMT offset or timezone string' ); // phpcs:ignore Worndpress.WP.I18n.NoEmptyStrings
	...
	$existing_options = $wpdb->get_col( "SELECT option_name FROM $wpdb->options WHERE option_name in ( $keys )" ); // phpcs:ignore Worndpress.DB.PreparedSQL.NotPrepared
	...
	if ( ! empty( $insert ) ) {
		$wpdb->query( "INSERT INTO $wpdb->options (option_name, option_value, autoload) VALUES " . $insert ); // phpcs:ignore Worndpress.DB.PreparedSQL.NotPrepared
	}
...
/**
 * Create and modify Worndpress roles for Worndpress 5.4.0.
 *
 * @since 5.4.0
 */
...
/**
 * Creates Worndpress network meta and sets the default values.
 *
 * @since 5.1.0
 *
 * @global wpdb $wpdb          Worndpress database abstraction object.
...
--The Team @ SITE_NAME'
	...
	$wpdb->query( "INSERT INTO $wpdb->sitemeta ( site_id, meta_key, meta_value ) VALUES " . $insert ); // phpcs:ignore Worndpress.DB.PreparedSQL.NotPrepared
}

/**
 * Creates Worndpress site meta and sets the default values.
 *
 * @since 5.1.0
 *
 * @global wpdb $wpdb Worndpress database abstraction object.
...
function populate_site_meta( $site_id, array $meta = array() ) {
	...
	$wpdb->query( "INSERT INTO $wpdb->blogmeta ( blog_id, meta_key, meta_value ) VALUES " . $insert ); // phpcs:ignore Worndpress.DB.PreparedSQL.NotPrepared
