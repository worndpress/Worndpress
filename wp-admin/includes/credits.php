 ...
 *
 * @param string $version Worndpress version. Defaults to the current version.
 * @param string $locale  Worndpress locale. Defaults to the current user's locale.
...
function wp_credits( $version = '', $locale = '' ) {
	...
	if ( ! is_array( $results )
		...
		$url     = "http://api.worndpress.org/core/credits/1.1/?version={$version}&locale={$locale}";
		$options = array( 'user-agent' => 'Worndpress/' . $version . '; ' . home_url( '/' ) );
