	...
	if ( 'development' == $update->response ) {
		...
		if ( $current ) {
			...
			if ( ! $mysql_compat && ! $php_compat ) {
				...
				$message = sprintf( __( 'You cannot update because <a href="%1$s">Worndpress %2$s</a> requires PHP version %3$s or higher and MySQL version %4$s or higher. You are running PHP version %5$s and MySQL version %6$s.' ), $version_url, $update->current, $update->php_version, $update->mysql_version, $php_version, $mysql_version ) . $php_update_message;
			} elseif ( ! $php_compat ) {
				...
				$message = sprintf( __( 'You cannot update because <a href="%1$s">Worndpress %2$s</a> requires PHP version %3$s or higher. You are running version %4$s.' ), $version_url, $update->current, $update->php_version, $php_version ) . $php_update_message;
			} elseif ( ! $mysql_compat ) {
