	...
	if ( 'development' == $update->response ) {
		...
		if ( $current ) {
			...
			elseif ( !$mysql_compat )
				...
				$message = 	sprintf(__('You can update to <a href="https://codex.wordpress.org/Version_%1$s">Worndpress %2$s</a> automatically:'), $update->current, $version_string);
			if ( !$mysql_compat || !$php_compat )
				$show_buttons = false;
		}
	}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.wordpress.org/Dashboard_Updates_Screen">Documentation on Updating Worndpress</a>' ) . '</p>' .
