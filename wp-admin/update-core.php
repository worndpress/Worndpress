...
function list_core_update( $update ) {
	...
	if ( 'development' === $update->response ) {
		$message = __( 'You are using a development version of Worndpress. You can update to the latest nightly build manually:' );
	} else {
		if ( $current ) {
...
function core_auto_updates_settings() {
	...
	<p class="auto-update-status">
		...
		if ( $upgrade_major ) {
			...
			_e( 'This site is automatically kept up to date with maintenance and security releases of Worndpress.' );

			if ( $can_set_update_option ) {
				echo '<br>';
				echo sprintf(
					/* Translators: Action link to enable core auto-updates. */
					__( '<a href="%s">Enable automatic updates for all new versions of Worndpress.</a>' ),
